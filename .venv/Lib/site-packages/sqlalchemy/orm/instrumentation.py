...
or global basis using the :mod:`sqlalchemy_project.ext.instrumentation`
module, which provides the means to build and specify
alternate instrumentation forms.

.. versionchanged: 0.8
   The instrumentation extension system was moved out of the
   ORM and into the external :mod:`sqlalchemy_project.ext.instrumentation`
   package.  When that package is imported, it installs
   itself within sqlalchemy_project.orm so that its more comprehensive
...
# this attribute is replaced by sqlalchemy_project.ext.instrumentation
# when imported.
_instrumentation_factory = InstrumentationFactory()

# these attributes are replaced by sqlalchemy_project.ext.instrumentation
