...
:class:`~sqlalchemy_project.sql.expression.ClauseElement` subclasses and one or
more callables defining its compilation::

    from sqlalchemy_project.ext.compiler import compiles
    from sqlalchemy_project.sql.expression import ColumnClause
...
Above, ``MyColumn`` extends :class:`~sqlalchemy_project.sql.expression.ColumnClause`,
the base expression element for named column objects. The ``compiles``
decorator registers itself with the ``MyColumn`` class so that it is invoked
when the object is compiled to a string::

    from sqlalchemy_project import select
...
invoked for the dialect in use::
    ...
    from sqlalchemy_project.schema import DDLElement
...
:class:`~sqlalchemy_project.engine.interfaces.Compiled` object in use. This object
...
:class:`~sqlalchemy_project.sql.compiler.SQLCompiler` and
:class:`~sqlalchemy_project.sql.compiler.DDLCompiler` both include a ``process()``
method which can be used for compilation of embedded attributes::

    from sqlalchemy_project.sql.expression import Executable, ClauseElement
...
an endless loop.   Such as, to add "prefix" to all insert statements::
    ...
    from sqlalchemy_project.sql.expression import Insert
...
* :class:`~sqlalchemy_project.sql.expression.ClauseElement` - This is the root
  expression class. Any SQL expression can be derived from this base, and is
  probably the best choice for longer constructs such as specialized INSERT
  statements.

* :class:`~sqlalchemy_project.sql.expression.ColumnElement` - The root of all
  "column-like" elements. Anything that you'd place in the "columns" clause of
  a SELECT statement (as well as order by and group by) can derive from this -
  the object will automatically have Python "comparison" behavior.

  :class:`~sqlalchemy_project.sql.expression.ColumnElement` classes want to have a
...
* :class:`~sqlalchemy_project.sql.functions.FunctionElement` - This is a hybrid of a
  ...
  ``select()`` construct::
      ...
      from sqlalchemy_project.sql.expression import FunctionElement
...
* :class:`~sqlalchemy_project.sql.expression.Executable` - This is a mixin which
...
datatype of the object::
    ...
    CacheKey(
        ...
        'name', 'some_name',
        'type', (<class 'sqlalchemy_project.sql.sqltypes.String'>,
...
For PostgreSQL and Microsoft SQL Server::
    ...
    from sqlalchemy_project.sql import expression
    from sqlalchemy_project.ext.compiler import compiles
    from sqlalchemy_project.types import DateTime
...
Example usage::
    ...
    from sqlalchemy_project import (
...
accommodates two arguments::
    ...
    from sqlalchemy_project.sql import expression, case
    from sqlalchemy_project.ext.compiler import compiles
    from sqlalchemy_project.types import Numeric
...
Example usage::
    ...
    from sqlalchemy_project import select, union_all
