...
def _infer_type_from_mapped(
    ...
    # NameExpr(query_expression [sqlalchemy_project.orm._orm_constructors.query_expression])  # noqa: E501
    # (Pdb) stmt.rvalue.callee.node
    # <mypy.nodes.FuncDef object at 0x7f8d92fb5940>
    # (Pdb) stmt.rvalue.callee.node.type
    # def [_T] (default_expr: sqlalchemy_project.sql.elements.ColumnElement[_T`-1] =) -> sqlalchemy_project.orm.base.Mapped[_T`-1]  # noqa: E501
    # sqlalchemy_project.orm.base.Mapped[_T`-1]
...
def extract_python_type_from_typeengine(
    ...
    if node.fullname == "sqlalchemy_project.sql.sqltypes.Enum" and type_args:
    ...
    assert node.has_base("sqlalchemy_project.sql.type_api.TypeEngine"), (
        "could not extract Python type from node: %s" % node
    )

    type_engine_sym = api.lookup_fully_qualified_or_none(
        "sqlalchemy_project.sql.type_api.TypeEngine"
    )
