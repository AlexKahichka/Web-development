...
JSON strings before being persisted::
    ...
    from sqlalchemy_project.types import TypeDecorator, VARCHAR
    import json
...
:mod:`sqlalchemy_project.ext.mutable` extension can be used
with any type whose target Python type may be mutable, including
:class:`.PickleType`, :class:`_postgresql.ARRAY`, etc.

When using the :mod:`sqlalchemy_project.ext.mutable` extension, the value itself
tracks all parents which reference it.  Below, we illustrate a simple
version of the :class:`.MutableDict` dictionary object, which applies
the :class:`.Mutable` mixin to a plain Python dictionary::

    from sqlalchemy_project.ext.mutable import Mutable
...
attribute. Such as, with classical table metadata::
    ...
    from sqlalchemy_project import Table, Column, Integer
...
will flag the attribute as "dirty" on the parent object::
    ...
    >>> from sqlalchemy_project import Session
...
The key to the :mod:`sqlalchemy_project.ext.mutable` extension relies upon the
...
``__setattr__`` to the :meth:`.MutableComposite.changed` method::
    ...
    import dataclasses
    from sqlalchemy_project.ext.mutable import MutableComposite
...
objects to each of the ``Vertex.start`` and ``Vertex.end`` attributes::
    ...
    from sqlalchemy_project.orm import DeclarativeBase, Mapped
    from sqlalchemy_project.orm import composite, mapped_column
