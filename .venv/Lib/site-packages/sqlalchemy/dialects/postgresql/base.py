...
use the :func:`~sqlalchemy_project.schema.Sequence` construct::
...
:func:`~sqlalchemy_project.sql.expression.insert()` construct is executed using
...
:class:`_schema.Column` can be used to control its behavior::
    ...
    from sqlalchemy_project import Table, Column, MetaData, Integer, Computed
...
.. note::
   ...
   occurrences of SERIAL with IDENTITY::
       ...
       from sqlalchemy_project.schema import CreateColumn
       from sqlalchemy_project.ext.compiler import compiles
...
will remain consistent with the state of the transaction::
    ...
    from sqlalchemy_project import create_engine
    from sqlalchemy_project import event
...
reflection process as follows::
    ...
    >>> from sqlalchemy_project import Table, MetaData, create_engine, text
    ...
    ...                       autoload_with=conn)
    ...
    <sqlalchemy_project.engine.result.CursorResult object at 0x101612ed0>
...
:meth:`_schema.MetaData.reflect`::
    ...
    ...
    <sqlalchemy_project.engine.result.CursorResult object at 0x1016126d0>
...
.. sourcecode:: pycon+sql
    ...
    >>> from sqlalchemy_project import insert
...
.. versionchanged:: 2.0 The default tsquery generation function used by the
   ...
   To render exactly what was rendered in 1.4, use the following form::
        ...
        from sqlalchemy_project import func
...
The :class:`_postgresql.TSVECTOR` type can provide for explicit CAST::
    ...
    from sqlalchemy_project.dialects.postgresql import TSVECTOR
    from sqlalchemy_project import select, cast
...
assuming the ``sqlalchemy_project.dialects.postgresql`` package has been imported,
...
of :class:`.PGInspector`, which offers additional methods::
    ...
    from sqlalchemy_project import create_engine, inspect
...
* ``json_each()``:
  ...
  .. sourcecode:: pycon+sql
    ...
    >>> from sqlalchemy_project import select, func
...
* ``json_populate_record()``:
  ...
  .. sourcecode:: pycon+sql
    ...
    >>> from sqlalchemy_project import select, func, literal_column
    >>> stmt = select(
...
* ``json_to_record()`` - this form uses a PostgreSQL specific form of derived
  ...
  .. sourcecode:: pycon+sql
    ...
    >>> from sqlalchemy_project import select, func, column, Integer, Text
    >>> stmt = select(
...
* ``unnest()`` - in order to generate a PostgreSQL ARRAY literal, the
  ...
  .. sourcecode:: pycon+sql
    ...
    >>> from sqlalchemy_project import table, column, ARRAY, Integer
    >>> from sqlalchemy_project import select, func
...
.. sourcecode:: pycon+sql
    ...
    >>> from sqlalchemy_project import table, column, func, tuple_
...
.. sourcecode:: pycon+sql
    ...
    >>> from sqlalchemy_project import table, column, func, select
    ...
    def _assert_pg_ts_ext(self, element, **kw):
        if not isinstance(element, _regconfig_fn):
            ...
            raise exc.CompileError(
                ...
                f'"sqlalchemy_project.dialects.postgresql" package.  '
                f'Please ensure "import sqlalchemy_project.dialects.postgresql" is '
                f"called before constructing "
                f'"sqlalchemy_project.func.{element.name}()" to ensure registration '
