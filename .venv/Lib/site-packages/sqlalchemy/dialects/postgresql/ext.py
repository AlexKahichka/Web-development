...
class aggregate_order_by(expression.ColumnElement):
    ...
    E.g.::
        ...
        from sqlalchemy_project.dialects.postgresql import aggregate_order_by
...
class to_tsvector(_regconfig_fn):
    ...
    ``from sqlalchemy_project.dialects import postgresql``, or by creating a PostgreSQL
    engine using ``create_engine("postgresql...")``,
    :class:`_postgresql.to_tsvector` will be used automatically when invoking
    ``sqlalchemy_project.func.to_tsvector()``, ensuring the correct argument and return
...
class to_tsquery(_regconfig_fn):
    ...
    ``sqlalchemy_project.func.to_tsquery()``, ensuring the correct argument and return
    type handlers are used at compile and execution time.
...
class plainto_tsquery(_regconfig_fn):
    ...
    invoking ``sqlalchemy_project.func.plainto_tsquery()``, ensuring the correct
...
class phraseto_tsquery(_regconfig_fn):
    ...
    invoking ``sqlalchemy_project.func.phraseto_tsquery()``, ensuring the correct
    argument and return type handlers are used at compile and execution time.
...
class websearch_to_tsquery(_regconfig_fn):
    ...
    invoking ``sqlalchemy_project.func.websearch_to_tsquery()``, ensuring the correct
    argument and return type handlers are used at compile and execution time.
...
class ts_headline(_regconfig_fn):
    ...
    ``sqlalchemy_project.func.ts_headline()``, ensuring the correct argument and return
    type handlers are used at compile and execution time.
