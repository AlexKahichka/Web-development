...
def pytest_addoption(parser):
    group = parser.getgroup("sqlalchemy_project")
...
def _log_sqlalchemy_info(session):
    import sqlalchemy_project
    from sqlalchemy_project import __version__
    from sqlalchemy_project import has_compiled_ext
    from sqlalchemy_project import _CYEXTENSION_MSG

    greet = "sqlalchemy_project installation"
    site = "no user site" if sys.flags.no_user_site else "user site loaded"
    msgs = [
        f"SQLAlchemy {__version__} ({site})",
        f"Path: {sqlalchemy_project.__file__}",
    ]

    if has_compiled_ext():
        from sqlalchemy_project import util
...
def pytest_collection_finish(session):
    if session.config.option.dump_pyannotate:
        ...
        lib_sqlalchemy = os.path.abspath("lib/sqlalchemy_project")

        def _filter(filename):
            filename = os.path.normpath(os.path.abspath(filename))
            if "lib/sqlalchemy_project" not in os.path.commonpath(
...
def _pytest_fn_decorator(target):
    ...
    from sqlalchemy_project import format_argspec_plus
    from sqlalchemy_project import inspect_getfullargspec
...
class PytestFixtureFunctions(plugin_base.FixtureFunctions):
    ...
    def combinations(self, *arg_sets, **kw):
        ...
        """
        from sqlalchemy_project import exclusions
